...
def fun(x, args):
    ...
    if ARGS.objectiveType == "abs":
        cardtotal = sum(weight[k]* np.abs( 0.5 * (1 + np.dot(cardcons([j/abs(j)*x[abs(j)-1] for j in clauses[k]]), FC_table[k]))) for k in range(len(clauses)) if (ctype[k]=='c'))
        ...
        cardtotal = sum(weight[k] * np.square( 0.5 * (1 + np.dot(cardcons([j/abs(j)*x[abs(j)-1] for j in clauses[k]]), FC_table[k]))) for k in range(len(clauses)) if (ctype[k]=='c'))
        ...
        cardtotal = sum(weight[k] * ( 0.5 * (1 + np.dot(cardcons([j/abs(j)*x[abs(j)-1] for j in clauses[k]]), FC_table[k]))) for k in range(len(clauses)) if (ctype[k]=='c'))
        ...
        cardtotal = sum( - weight[k] * np.log( 1.000001 - 0.5 * (1 + np.dot(cardcons([j/abs(j)*x[abs(j)-1] for j in clauses[k]]), FC_table[k]))) for k in range(len(clauses)) if (ctype[k]=='c'))
        xortotal = sum(weight[k] * np.log(1 - 0.5 * (1 + prod(j/abs(j)*x[abs(j)-1] for j in clauses[k]))) for k in range(len(clauses)) if (ctype[k]=='x'))
